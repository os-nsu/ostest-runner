...
class Network:
	...
	def get_task(self):
		...
		try:
			...
			logging.error(f"No task available: got exception: {e}")
			return

		if response:
			return response
		else:
			logging.info(f"No task available: response status code: {response.status_code}")
	...
	def post_results(self, result):
		...
		if response:
			logging.info(f"Sent results to backend server successfully, return code: {response.status_code}")
		else:
			logging.error(f"Couldn't send results to backend server, return code: {response.status_code}")
...
class NetworkWithAuth:
	...
	def __refreshAuthToken(self):
		...
		if response:
			...
			logging.info(f"refresh token success, return code: {response.status_code}")
			...
			logging.error(f"refresh token error, resetting auth, return code: {response.status_code}")
			self.__token_type = None
			self.__authToken = None
	...
	def __get_auth_token(self):
		...
		if response:
			response_json = response.json()
			self.__last_token_update = time.time()
			logging.info(f"auth success, return code: {response.status_code}")
			...
			logging.error(f"auth error, return code: {response.status_code}")
			return
	...
	def post_results(self, result):
		...
		if response:
			...
			if (response.status_code == 401):
				self.__token_type = None
				self.__authToken = None
				logging.info(f"logging again and sending results")
